...
const int JOIN_TIMEOUT_MS = 10000;
const int DEFAULT_BEACON_TIMEOUT = 30;
...
void initialize_wifi() {
    ...
    wifi_mode_t existing_wifi_mode;
    if (ESP_OK == esp_wifi_get_mode(&existing_wifi_mode) && existing_wifi_mode == WIFI_MODE_STA) {
        wifi_config_t saved_config;
        if (ESP_OK == esp_wifi_get_config(WIFI_IF_STA, &saved_config)) {
            ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &saved_config));
            ESP_ERROR_CHECK(esp_wifi_set_inactive_time(WIFI_IF_STA, DEFAULT_BEACON_TIMEOUT));
            esp_wifi_connect();
        } else {
            ESP_LOGE("wifi", "unable to get saved wifi config");
        }
    } else {
        ESP_LOGE("wifi", "Unable to get saved wifi mode - %d", existing_wifi_mode);
    }
